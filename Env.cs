...
namespace JKLApp
{
    public class Env
    {
        ...
        // or cases where the binds and expr sequences are of incompatible lengths. This latter
        // situation could arise if the arity of a function supplied to map by userland code doesn't 
        // match the number of supplied sequences.
        public Env(Env outer, JKLSequence binds, JKLSequence exprs)
        {
            ...
            for (bindsProcessed = 0; bindsProcessed < binds.Count(); bindsProcessed++)
            {
                ...
                else if (binds[bindsProcessed] is JKLSym symbol)
                {
                    ...
                    {
                        throw new JKLEvalError("More parameters (bindings) than supplied values (expressions): " + binds.ToString(true) + " ... " + exprs.ToString(true));
                    }
            ...
            if (bindsProcessed < exprs.Count())
            {
                ...
                throw new JKLEvalError("Fewer parameters (bindings) than supplied values (expressions): " + binds.ToString(true) + " ... " + exprs.ToString(true));
            }
        }
