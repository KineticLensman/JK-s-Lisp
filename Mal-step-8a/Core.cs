...
namespace Mal
{
    class Core
    {
        ...
        public static MalFunc BuiltinCons = new MalFunc(args =>
        {
            ...
            // TODO - genericise
            if (args[1] is MalList listToCons)
            {
                newList.Add(args[0]);
                for (var i = 0; i < listToCons.Count(); i++)
                {
                    newList.Add(listToCons[i]);
                }
                return newList;
            }
            else if (args[1] is MalVector VectorToCons)
            {
                newList.Add(args[0]);
                for (var i = 0; i < VectorToCons.Count(); i++)
                {
                    newList.Add(VectorToCons[i]);
                }
                return newList;
            }
            else
        ...
        // of all the list sequences. Unlike the guide, this handles hashmaps.
        public static MalFunc BuiltinConcat = new MalFunc(args =>
        {
            MalList newList = new MalList();
            for (var i = 0; i < args.Count(); i++)
            {
                if (args[i] is MalSeq seqToConcat)
                {
