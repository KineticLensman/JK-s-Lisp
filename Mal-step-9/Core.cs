...
namespace Mal
{
    class Core
    {
        ...
        public static MalFunc BuiltinHashMap = new MalFunc(args =>
        {
            ...
            if (argCount % 2 != 0)
            {
                throw new MalEvalError("hashmap requires an even number of elements forming key value pairs '" + args.ToString(true) + "'");
            }

            MalHashMap builtHashMap = new MalHashMap();
            for (int i = 0; i < argCount; i += 2)
            {
                ...
                {
                    throw new MalEvalError("hashmap: Expected a keyword or string as a HashMap key but got '" + key.ToString(true) + "'");
                }
            }
            return builtHashMap;
        });
        ...
        // mal values are immutable), and a new hash-map containing the old hash-maps key/values plus the
        ...
        public static MalFunc BuiltinThrow = new MalFunc(args =>
        {
            // Part of the Mal exception handling mechanism.
            CheckArgCount(args, 1, "throw");

            if ((object)args[0] is MalVal mv)
            {
                throw new MalMalError(mv);
            }
            throw new MalEvalError("throw expected Mal value but got '" + args[0].ToString(true) + "'");
        });

        // -------------------- Map, Apply and friends ---------------------------------------------

        public static MalFunc BuiltinMap = new MalFunc(args =>
        {
            CheckArgCount(args, 2, "map");
            if (args[0] is MalFunc f)
            {
                if (args[1] is MalSequence ms)
                {
                    MalList mapResult = new MalList();
                    for (var i = 0; i < ms.Count(); i++)
                    {
                        // Make the arg into an arg list for function application.
                        MalList ml = new MalList();
                        ml.Add(ms[i]);
                ...
                throw new MalEvalError("map: expected sequence but got '" + args[1].ToString(true) + "'");
            }
            throw new MalEvalError("map: expected function but got '" + args[0].ToString(true) + "'");
        });

        public static MalFunc BuiltinApply = new MalFunc(args =>
        {
            // E.g. (apply + 4 [5]) ==> 9
            int argCount = args.Count();
            if (argCount == 0)
            {
                throw new MalEvalError("apply - expected function and arguments");
            }
            if (argCount == 1)
            {
                throw new MalEvalError("apply '" + args[0].ToString(true) + "': no args to apply");
            }
            if (args[0] is MalFunc f)
            {
                MalList ApplyCall = new MalList();
                for (var i = 1; i < argCount; i++)
                {
                    // Add all of the args to the apply call
                    if (args[i] is MalSequence ms)
                    {
            ...
            throw new MalEvalError("apply: expected function but got '" + args[0].ToString(true) + "'");
        });
